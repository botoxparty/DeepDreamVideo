...
def extractVideo(inputdir, outputdir):
    print(subprocess.Popen('ffmpeg -i ' + inputdir + ' -f image2 '
                           + outputdir + '/%08d.png', shell=True,
...
def deepdream(
    ...
    **step_params
    ...
    for i in range(octave_n - 1):
        octaves.append(nd.zoom(octaves[-1], (1, 1.0 / octave_scale, 1.0
...
def deepdream_guided(
    ...
    **step_params
    ...
    for (octave, octave_base) in enumerate(octaves[::-1]):
        ...
        src.data[0] = octave_base + detail
        for i in range(iter_n):
            make_step_guided(net, end=end, clip=clip,
...
def main(
    ...
    nrframes = len([name for name in os.listdir(input)
    ...
    frame = np.float32(PIL.Image.open(input + '/%08d.%s' % (frame_i,
                       image_type)))
    if preview is not 0:
        frame = np.float32(resizePicture(input + '/%08d.%s' % (frame_i,
                           image_type), preview))
    ...
    for i in range(frame_i, nrframes):
        ...
        if guide_image is None:
            ...
            guide_features = prepare_guide(net,
                    PIL.Image.open(guide_image), end=endparam)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--output',
                        ...
                        , required=True)
    parser.add_argument('-it', '--image_type',
    ...
    parser.add_argument('-t', '--model_path', dest='model_path',
    ...
    parser.add_argument('-p', '--preview', type=int, required=False,
                        help='Preview image width. Default: 0')
    parser.add_argument('-oct', '--octaves', type=int, required=False,
                        help='Octaves. Default: 4')
    parser.add_argument('-octs', '--octavescale', type=float,
                        required=False,
                        help='Octave Scale. Default: 1.4')
    parser.add_argument('-itr', '--iterations', type=int,
                        required=False, help='Iterations. Default: 10')
    parser.add_argument('-j', '--jitter', type=int, required=False,
                        help='Jitter. Default: 32')
    parser.add_argument('-z', '--zoom', type=int, required=False,
                        help='Zoom in Amount. Default: 1')
    parser.add_argument('-s', '--stepsize', type=float, required=False,
                        help='Step Size. Default: 1.5')
    parser.add_argument('-b', '--blend', type=str, required=False,
    ...
    parser.add_argument('-v', '--verbose', type=int, required=False,
                        help='verbosity [0-3]')
    parser.add_argument('-gi', '--guide_image', required=False,
                        help='path to guide image')
    parser.add_argument('-sf', '--start_frame', type=int,
                        required=False, help='starting frame nr')
    parser.add_argument('-ef', '--end_frame', type=int, required=False,
                        help='end frame nr')
    parser.add_argument('-e', '--extract', type=int, required=False,
